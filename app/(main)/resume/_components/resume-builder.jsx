...
// Utility to strip oklch() usage from style attributes and from raw CSS
function stripOklchStyles(html) {
  // Remove oklch color declarations from inline style attributes
  html = html.replace(
    /style="([^"]*?)oklch\([^)]+?\)([^"]*?)"/gi,
    (match, before, after) => {
  ...
  html = html.replace(/background(-color)?\s*:\s*oklch\([^)]+?\)\s*;?/gi, "");
  return html;
}
